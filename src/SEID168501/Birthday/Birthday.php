...
class Birthday extends Database
{
    ...
    public function index(){
        ...
        $query = "SELECT * FROM `tbl_birthday` WHERE is_trashed = 'No'";
    ...
    public function trash(){
        $query = "UPDATE `tbl_birthday` SET is_trashed = NOW() WHERE `id` = $this->id;";
    ...
    public function recover(){
        $query = "UPDATE `tbl_birthday` SET is_trashed = 'No' WHERE `id` = $this->id;";
    ...
    public function delete(){
        $query = "DELETE FROM `tbl_birthday` WHERE `id` = $this->id;";
    ...
    public function search($requestArray){
        $sql = "";
        if( isset($requestArray['byName']) && isset($requestArray['byDob']) )  $sql = "SELECT * FROM `tbl_birthday` WHERE `is_trashed` ='No' AND (`name` LIKE '%".$requestArray['search']."%' OR `dob` LIKE '%".$requestArray['search']."%')";
        if(isset($requestArray['byName']) && !isset($requestArray['byDob']) ) $sql = "SELECT * FROM `tbl_birthday` WHERE `is_trashed` ='No' AND `name` LIKE '%".$requestArray['search']."%'";
        if(!isset($requestArray['byName']) && isset($requestArray['byDob']) )  $sql = "SELECT * FROM `tbl_birthday` WHERE `is_trashed` ='No' AND `dob` LIKE '%".$requestArray['search']."%'";
    ...
    public function getAllKeywords()
    {
        ...
        foreach ($allData as $oneData) {
            $_allKeywords[] = trim($oneData->dob);
        }
        $allData = $this->index();

        foreach ($allData as $oneData) {

            $eachString= strip_tags($oneData->dob);
            $eachString=trim( $eachString);
            $eachString= preg_replace( "/\r|\n/", " ", $eachString);
            $eachString= str_replace("&nbsp;","",  $eachString);
            $WordsArr = explode(" ", $eachString);
    ...
    public function indexPaginator($page=1,$itemsPerPage=3){
        ...
        $sql = "SELECT * from tbl_birthday  WHERE is_trashed = 'No' LIMIT $start,$itemsPerPage";
    ...
    public function trashList(){
        ...
        $sql = "Select * from tbl_birthday where is_trashed <> 'No'";
